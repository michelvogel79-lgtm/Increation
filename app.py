...
/* Tabs deutlich größer & kreativer */
...
.stTabs [aria-selected="true"] {
    ...
    box-shadow: 0 8px 25px rgba(74, 144, 226, 0.5), inset 0 1px 0 rgba(255, 255, 255, 0.3) !important;
}
...
[data-testid="stChatInput"] {
    ...
    box-shadow: 0 10px 40px rgba(74, 144, 226, 0.4) !important;
}
...
/* Padding unten, damit Inhalt nicht hinter Input verschwindet */
...
.donation-btn {
    ...
    box-shadow: 0 4px 15px rgba(74, 144, 226, 0.4), inset 0 1px 0 rgba(255, 255, 255, 0.3);
...
with tab1:
    ...
    with st.sidebar:
        ...
        chat_model = st.selectbox("🎯 Modell wählen:", [
            ...
            "gemma2-9b-it",
            "mixtral-8x7b-32768"
        ], key="chat_model", help="Wähle das KI-Modell für deine Gespräche")
        
        ausfuehrlichkeit = st.selectbox("📝 Antwort-Stil:", [
            "✨ Kurz & knackig",
            "📄 Normal",
            "📚 Sehr ausführlich"
        ], key="ausfuehrlichkeit", index=2, help="Wie ausführlich soll die KI antworten?")
        
        max_tokens_setting = st.slider("🔢 Max. Tokens:", 1000, 8000, 4000, 500, help="Mehr Tokens = längere Antworten")
    ...
    # System-Prompts
    system_prompts = {
        "✨ Kurz & knackig": "Du bist ein hilfreicher Assistent. Antworte kurz, präzise und auf den Punkt. Maximal 3-4 Sätze, keine unnötigen Details.",
        "📄 Normal": "Du bist ein hilfreicher Assistent. Antworte freundlich und informativ mit den wichtigsten Informationen.",
        "📚 Sehr ausführlich": "Du bist ein extrem hilfreicher und kompetenter Assistent. Antworte immer sehr ausführlich und detailliert. Verwende:\n- Klare Überschriften und Absätze\n- Aufzählungen und Listen\n- Konkrete Beispiele\n- Hintergrundinformationen\n- Vergleiche wo sinnvoll\nGib so viele relevante Details wie möglich, ohne belanglos zu werden. Strukturiere deine Antwort professionell wie ein Fachtext."
    }
    ...
    with chat_container:
        if not st.session_state.messages:
            ...
            <div class="kreativ-box" style="text-align: center;">
                ...
                Stell mir Fragen zu jedem Thema – von Wissenschaft über Programmierung bis hin zu kreativem Schreiben.<br>
    ...
    # Chat Input (fixiert unten via CSS)
...
with tab2:
    ...
    <div class="kreativ-box">
        ...
        🎨 Beschreibe dein Wunschbild so detailliert wie möglich. Je genauer deine Beschreibung, desto besser das Ergebnis!
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        img_prompt = st.text_area(
            " ",
            placeholder="z.B. Ein majestätischer Drache über einer mittelalterlichen Stadt bei Sonnenuntergang, epische Fantasy-Szene, detailliert, 4K",
            height=120,
    ...
    with col2:
        ...
        img_style = st.selectbox("🎨 Stil:", [
            "realistisch", "künstlerisch", "anime", "digital art", "3D render", "Ölgemälde", "Aquarell", "Cyberpunk"
        ])
...
with tab3:
    ...
    <div class="kreativ-box">
        <p style="color: #B8D4F0; margin: 0;">
        💻 Beschreibe, was der Code tun soll – die KI generiert dir sauberen, kommentierten Code in deiner Wunschsprache.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        code_request = st.text_area(
            " ",
            placeholder="z.B. Eine Python-Funktion, die alle Primzahlen bis zu einer gegebenen Zahl n zurückgibt",
            height=120,
            label_visibility="collapsed"
        )
    ...
    with col2:
        ...
        code_language = st.selectbox("💻 Sprache:", [
            "Python", "JavaScript", "HTML/CSS", "Java", "C++", "SQL", "TypeScript", "Go", "Rust", "PHP"
        ])
    ...
    if st.button("⚡ Code generieren", use_container_width=True, key="gen_code", type="primary"):
        if code_request:
            with st.spinner("💻 Code wird geschrieben..."):
                try:
                    kommentar_stil = {
                        "Anfänger (viele Kommentare)": "mit ausführlichen Kommentaren für jeden Schritt, ideal für Lernende",
                        "Fortgeschritten (normale Kommentare)": "mit normalen Kommentaren, sauber und verständlich",
                        "Profi (minimal Kommentare)": "minimal kommentiert, produktionsreif und effizient"
                    }
                    
                    full_prompt = f"""Schreibe {code_language} Code für: {code_request}.
Stil: {kommentar_stil[code_erfahrung]}.
Gib nur den fertigen Code zurück, ohne zusätzliche Erklärungen drumherum."""
                    ...
                    if "```" in code:
                        parts = code.split("```")
                        # Suche nach dem Code-Block
                        for i, part in enumerate(parts):
                            if i % 2 == 1:  # Code-Block
                                # Entferne Sprach-Tag am Anfang
                                lines = part.strip().split('\n', 1)
                                if len(lines) > 1 and lines[0].strip() in ['python', 'javascript', 'html', 'css', 'java', 'cpp', 'c++', 'sql', 'typescript', 'go', 'rust', 'php', 'py', 'js', 'ts']:
                                    code_clean = lines[1].strip()
                                else:
                                    code_clean = part.strip()
                                break
                        else:
                    ...
                    st.download_button(
                        "💾 Code herunterladen",
                        code_clean,
                        f"increation_code.{code_language.lower().replace('++', 'pp').replace('#', '')}",
                        "text/plain",
                        use_container_width=True
                    )
                except Exception as e:
...
st.sidebar.markdown("""
<div class="kreativ-box" style="padding: 15px; margin: 0;">
    <p style="color: #6BB6FF; font-weight: 700; margin: 0 0 10px 0;">🧠 increation KI Studio</p>
    ...
    <p style="color: #B8D4F0; font-size: 13px; margin: 5px 0;">🚀 Ultra-schnelle Antworten</p>
</div>
""", unsafe_allow_html=True)
